...
function auditer() {
  ...
  if (process.argv.includes('--manques-md')) {
    ...
    out.push('---');
    out.push('');
    out.push('## Incohérences fiche / variante — à trancher');
    out.push('');
    out.push('Repérées en sourçant : la **puissance de la fiche ne correspond pas à la');
    out.push('voiture que son nom annonce**. Ce sont des décisions de périmètre, pas des');
    out.push('trous de données — les combler mécaniquement figerait l\'erreur. C\'est la même');
    out.push('famille que les bugs Giulia/Quadrifoglio et M2 CS/M4 CSL.');
    out.push('');
    out.push('| Fiche | Le problème | Options |');
    out.push('|---|---|---|');
    out.push('| **Peugeot 106** (générique) | Porte 120 ch / 145 Nm, soit les chiffres de la **106 GTI**. Une 106 de base fait 45 à 60 ch. | Soit la fiche devient une 106 de base (et il faut ses chiffres), soit elle assume d\'être la sportive — mais alors elle double `peugeot-106-gti`. |');
    out.push('| **Simca 1000 Rallye 2** | Porte 103 ch, qui est la puissance de la **Rallye 3** (déc. 1977). La Rallye 2 développe 82 ch / 11 mkg. | Renommer en Rallye 3, ou ramener à 82 ch et prendre le couple de la Rallye 2. |');
    out.push('| **Fiat 500 (Nuova)** | Porte 23 ch, qui est la **500 R** (594 cm³). La Nuova 499 cm³ fait 18 à 21,5 ch. | Trancher quelle version la fiche représente avant de chercher son couple. |');
    out.push('| **Lancia Rally 037** | `ch:205` est la **Stradale** (route), mais `kg:960` est le poids de la version **Groupe B** de course. La Stradale pèse ~1 170 kg. | Le couple ajouté (234 Nm) est celui de la Stradale : aligner la masse, ou assumer la fiche « voiture de course ». |');
    out.push('| **Renault Scénic** | `ch:170` est le **Scénic E-Tech électrique** (125 kW), mais `cyl:1.3` est le thermique (qui plafonne à 160 ch). Ajouter le couple de l\'électrique contre une cylindrée de 1,3 L produirait un Nm/L absurde. | Trancher : fiche thermique ou fiche électrique ? |');
    out.push('| **Ariel Atom** | `ch:350` ne correspond à aucune version des bases : Atom 3.5 = 242 ch, Atom 4 = 320 ch, Atom 4R = 400 ch. | Identifier la version visée (3.5R compressée ?) avant de chercher son couple. |');
    out.push('');
    ...
    out.push('| **Micro-citadines d\'époque** (Fiat 500/600/126, BMW Isetta, Trabant 601, DAF 33) | Le couple n\'était tout simplement pas communiqué à l\'époque pour ces voitures. Piste : les revues techniques (RTA) et les notices constructeur d\'origine. |');
    out.push('');
